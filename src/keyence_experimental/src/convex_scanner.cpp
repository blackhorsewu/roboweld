...
************************************************************************************************/
          ...
          int valid_begin = 0;
          int maxDoubleDot1i, maxDoubleDot2i;
 ...
 ********************************************************************************************/
            ...
            {   // then this is a piece of normal data
              ...
              else
              {
                ...
                else if (j >= ((((((w+m)+m)+m)+m)+1)+1))
                {
                  ...
                  if (avgddavgz[((j-m)-m)-m] < minDoubleDotZ)
                  {
                    minDoubleDotZ = avgddavgz[((j-m)-m)-m];
                    minDoubleDotj = ((j-m)-m)-m;
                  }
                  if (write_X_file)
                  {
  ...
  ROS_INFO_STREAM("Min DD position: " << minDoubleDotj << " Line no.: " << line_no);
 ...
 ********************************************************************************************/
          ...
          { // Second Loop through the Scan Line
            ...
            {   // then this is a piece of normal data
              ...
              {
                if ((i < (minDoubleDotj-10)) && (avgddavgz[i] > maxDoubleDot1Z))
                {
                  maxDoubleDot1Z = avgddavgz[i];
                  maxDoubleDot1i = i;
                }
                if ((i > (minDoubleDotj+10)) && (avgddavgz[i] > maxDoubleDot2Z))
                {
  ...
  ROS_INFO_STREAM("Left Max DD position: " << maxDoubleDot1i);
  ROS_INFO_STREAM("Right Max DD position: " << maxDoubleDot2i);
 ...
 ********************************************************************************************/
          ...
          int begin = maxDoubleDot1i + valid_begin;
          int end = maxDoubleDot2i + valid_begin;
          ...
          Bot.x = pointcloud[minDoubleDotj + valid_begin].x;
          Bot.y = pointcloud[minDoubleDotj + valid_begin].y;
          Bot.z = pointcloud[minDoubleDotj + valid_begin].z;

...
static double minTime(const std::vector<double>& pose_a, const std::vector<double>& pose_b, double max_vel)
{
  ...
  std::transform(pose_a.begin(),
                 pose_a.end(),
                 pose_b.begin(),
                 std::back_inserter(diff),
                 [max_vel](double a, double b)
                   {
...
bool descartes_utilities::toRosJointPoints(
                     ...
                     std::vector<trajectory_msgs::JointTrajectoryPoint>& out)
{
  if (default_joint_vel <= 0.0)
  {
    CONSOLE_BRIDGE_logError(
       "%s: Invalid value for default joint velocity. Must be > 0 (radians/second)", 
       __FUNCTION__);
    return false;
  }
  ...
  {
    CONSOLE_BRIDGE_logError("%s: Default joint velocity of %f exceeds assumed limit of %f.",
                            __FUNCTION__, default_joint_vel,
  ...
  for (std::size_t i = 0; i < joint_traj.size(); ++i)
  {
    if (!joint_traj[i])
    {
      CONSOLE_BRIDGE_logError("%s: Input trajectory contained null pointer at index %lu",
      __FUNCTION__, static_cast<unsigned long>(i));
      return false;
    }
    ...
    {
      CONSOLE_BRIDGE_logError(
      "%s: Failed to extract joint positions from input trajectory at index %lu", __FUNCTION__,
    ...
    // Descartes has no internal representation of velocity, acceleration, 
    // or effort so we fill these field with zeros.
...
ROS_INFO_STREAM("Way point " << i << " dt: " << dt);
